...
int ecall_create_report(
    ...
    INFO_OUTPUT_CONSOLE("DEBUG---- Request ID: %s, pubkey_list_hash: %s", request_id, pubkey_list_hash);
    INFO_OUTPUT_CONSOLE("DEBUG---- Request ID: %s, key_meta_hash: %s", request_id, key_meta_hash.c_str());
    INFO_OUTPUT_CONSOLE("DEBUG---- Request ID: %s, server_pubkey: %s", request_id, server_pubkey.c_str());
...
void clear_context(int duration) {
    ...
    for (const auto &pair: g_keyContext_list) {
        INFO_OUTPUT_CONSOLE("clear_context: Key in g_keyContext_list: %s, status: %d, finished_time: %ld",
                            pair.first.c_str(), pair.second->key_status, pair.second->finished_time);
    }
    ...
    for (auto it = g_keyContext_list.begin(); it != g_keyContext_list.end();) {
        ...
        if (it->second->key_status == eKeyStatus_Error || it->second->key_status == eKeyStatus_Unknown) {
            INFO_OUTPUT_CONSOLE("clear_context: Deleting key %s due to error or unknown status %d", it->first.c_str(),
                                it->second->key_status);
            delete it->second;
            it = g_keyContext_list.erase(it);
        }
            ...
            INFO_OUTPUT_CONSOLE("clear_context: Deleting key %s due to duration exceeded, status %d", it->first.c_str(),
                                it->second->key_status);
            delete it->second;
            it = g_keyContext_list.erase(it);
        } else {
